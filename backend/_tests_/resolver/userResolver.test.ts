...
describe("userResolver", () => {
    ...
    describe("Mutations", () => {
        ...
        it("Mutation: updateUser - Should update a user", async () => {
            const updatedUserData = { username: "UpdatedUser", email: "updatedUser@example.com", password: "newPassword" };
            ...
            const result = await userResolver.Mutation.updateUser(null, { userID: "1", ...updatedUserData }, mockContext as unknown as Context);
        ...
        it("Mutation: deleteUser - Should delete a user", async () => {
            ...
            const result = await userResolver.Mutation.deleteUser(null, { userID: "1" }, mockContext as unknown as Context);
    
            expect(mockContext.prisma.user.findUnique).toHaveBeenCalledWith({ where: { userID: "1" } });
            expect(mockContext.prisma.user.delete).toHaveBeenCalledWith({ where: { userID: "1" } });
        ...
        it("Mutation: addMovieToFavorite - Should add a movie to a user's favorites", async () => {
            const userID = "1";
            const imdbID = "movie1";
            const mockUser = { userID, username: "User1", email: "user1@example.com" };
    
            mockContext.prisma.user.findUnique.mockResolvedValue(mockUser);
            mockContext.prisma.userFavorites.create.mockResolvedValue({ userID, imdbID });
            mockContext.prisma.library.findMany.mockResolvedValue([]);
            mockContext.prisma.libraryMovie.findMany.mockResolvedValue([]);
            mockContext.prisma.userFavorites.findMany.mockResolvedValue([]);
    
            const result = await userResolver.Mutation.addMovieToFavorite(null, { userID, imdbID }, mockContext as unknown as Context);
        ...
        it("Mutation: removeMovieFromFavorite - Should remove a movie from a user's favorites", async () => {
            ...
            mockContext.prisma.user.findUnique.mockResolvedValue(mockUser);
            mockContext.prisma.userFavorites.delete.mockResolvedValue({ userID, imdbID });
            mockContext.prisma.library.findMany.mockResolvedValue([]);
            mockContext.prisma.libraryMovie.findMany.mockResolvedValue([]);
            mockContext.prisma.userFavorites.findMany.mockResolvedValue([]);
    
            const result = await userResolver.Mutation.removeMovieFromFavorite(null, { userID, imdbID }, mockContext as unknown as Context);
        ...
        it("Mutation: addLibrary - Should add a new library for the user", async () => {
            ...
            mockContext.prisma.library.create.mockResolvedValue({ userID, name: libraryName });
            mockContext.prisma.user.findUnique.mockResolvedValue(mockUser);
            mockContext.prisma.library.findMany.mockResolvedValue([{ userID, name: libraryName }]);
            mockContext.prisma.libraryMovie.findMany.mockResolvedValue([]);
            mockContext.prisma.userFavorites.findMany.mockResolvedValue([]);
    
            const result = await userResolver.Mutation.addLibrary(null, { userID, libraryName }, mockContext as unknown as Context);
        ...
        it("Mutation: addLibrary - Should remove a library based on libraryID", async () => {
            ...
            const result = await userResolver.Mutation.removeLibrary(null, { userID, libraryID }, mockContext as unknown as Context);
    
            expect(mockContext.prisma.library.delete).toHaveBeenCalledWith({ where: { libraryID } });
            expect(result).toEqual(expect.objectContaining({
                userID,
                email: "user1@example.com",
                library: [],
